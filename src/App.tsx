...
function useTheme() {
  const [theme, setTheme] = useState<Theme>(
    // Storage key predates the rename and is deliberately left alone: changing
    // it would silently drop every existing user's saved preference.
...
/** Brand lockup. Both marks ship; CSS shows whichever suits the active theme. */
function Logo() {
  return (
    <a href="/" className="flex shrink-0 items-center" aria-label={STRINGS.appName}>
      <img
        src="/brand/logo.png"
        alt={STRINGS.appName}
        width={275}
        height={128}
        className="logo-on-light h-9 w-auto"
      />
      <img
